...
class InvariantMessagePasser(torch.nn.Module):
    ...
    def __init__(self, hypers: Dict, all_species: List[int], device: str = "cpu") -> None:
        ...
        self.radial_basis_calculator = RadialBasis(hypers["radial_basis"], all_species, device)
    ...
    def forward(
        ...
        for l, normalizer_l in enumerate(self.pooling_normalization):
            ...
            densities_l = torch.zeros((n_atoms, spherical_harmonics_l.shape[1], radial_basis_l.shape[1]), device=radial_basis_l.device, dtype=radial_basis_l.dtype)
            densities_l.index_add_(
                dim=0,
                index=centers,
                source=spherical_harmonics_l*radial_basis_l.unsqueeze(1)*initial_center_embedding.block().values[neighbors][:, :, :radial_basis_l.shape[1]]
            )            
            ...
            blocks.append(
                TensorBlock(
                    ...
                    properties = Labels.range("properties", densities_l.shape[2]).to(device=densities_l.device)
                )
            )
