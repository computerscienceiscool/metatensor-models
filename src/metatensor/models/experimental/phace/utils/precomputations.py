...
class Precomputer(torch.nn.Module):
    def __init__(self, l_max, normalize=True):
        ...
        self.spherical_harmonics_calculator = sphericart.torch.SphericalHarmonics(l_max, normalized=True)

    def forward(
        self, positions, cells, species, cell_shifts, centers, pairs, structure_centers, structure_pairs, structure_offsets
    ):  
        ...
        if self.normalize: spherical_harmonics = spherical_harmonics * (4.0*torch.pi)**(0.5)  # normalize them
        ...
        spherical_harmonics_blocks = [
            TensorBlock(
                ...
                properties=Labels.range("properties", 1).to(device=cartesian_vectors.values.device)
            ) for l, spherical_harmonics_l in enumerate(spherical_harmonics)
        ]
        spherical_harmonics_map = TensorMap(
            keys=Labels(
                names=["o3_lambda"],
                values=torch.arange(len(spherical_harmonics_blocks), device=r.device).reshape(len(spherical_harmonics_blocks), 1)
            ),
        ...
        r_block = TensorBlock(
            ...
            samples=cartesian_vectors.samples,
            components=[],
            properties=Labels.range("properties", 1).to(device=r.device)
        )
...
def get_cartesian_vectors(positions, cells, species, cell_shifts, centers, pairs, structure_centers, structure_pairs, structure_offsets):
    ...
    block = TensorBlock(
        ...
        samples = Labels(
            names = ["system", "atom", "neighbor", "center_type", "species_neighbor", "cell_x", "cell_y", "cell_z"],
            values = labels
        ),
        components = [
            Labels(
        ...
        properties = Labels.single().to(device=direction_vectors.device)
    )
