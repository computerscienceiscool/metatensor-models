...
def train_model(
    ...
    """
    Train an atomistic machine learning model using options.
    ...
    # Resolves all interpolations in the options with explicit values. Especially
    # crucial to have a FIXED seed.
    OmegaConf.resolve(options)
    ...
    OmegaConf.save(config=options, f=Path(checkpoint_dir) / "options_restart.yaml")
    ...
    output_checked = Path(check_suffix(fname=output, suffix=".pt"))
